...
Mac 側から検証を実行します。このコマンドは `third_party/vagrant_utm` から UTM provider をビルドして Vagrant の project-local plugin として入れ、VM の作成または起動をしてから、BLE HID の検証を実行します。
